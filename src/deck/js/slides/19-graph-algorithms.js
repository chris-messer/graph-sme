...
DECK.registerViz('19-graph-algorithms', (slide) => {
  ...
  /* A6 out to C6, crossing both bridges — five hops end to end. */
  const WALK = [5, 1, 6, 7, 12, 17];
  const ON_PATH = new Set(WALK);
  const PATH_EDGES = new Set(WALK.slice(1).map((id, i) => key(WALK[i], id)));
  ...
  const COPY = {
    ...
    pathfinding: {
      ...
      caption: 'A6 → C6 · five hops, and the route is the answer',
  ...
  function annotate(mode, at, animate) {
    ...
    if (mode === 'pathfinding') {
      ...
      WALK.slice(1).forEach((id, i) => {
        const s = at.get(WALK[i]);
        const t = at.get(id);
