/* 16 · the problem space — one frontier, two independent controls. The seed
   control decides whether the answer is right; the depth control decides how
   much comes back, and therefore whether the agent can read it at all. Both act
   on the same picture, which is the point of the slide. */
DECK.registerViz('16-problem-space', (slide) => {
  const SEED_GRAPH = (() => {
    const nodes = [];
    const links = [];
  ...
  function paint() {
    ...
    const returned = reachedRight.size + reachedWrong.size;
    const inFrontier = id => reachedRight.has(id) || reachedWrong.has(id);
    const edges = view.links.filter(d => inFrontier(endId(d.source)) && inFrontier(endId(d.target))).length;
    const share = returned / view.nodes.length;
    ...
    if (callout) {
      ...
      if (reachedRight.has('h1')) extra.push('Everything attached to the supernode is in the answer now.');
      else if (reachedRight.has('hub')) extra.push('The supernode is in the frontier. One more hop and everything attached to it returns too.');
      callout.innerHTML = `<b>${method.label} · ${state.hops} hop${state.hops > 1 ? 's' : ''}</b>`
        + method.note + (extra.length ? ' ' + extra.join(' ') : '');
    }

    const load = slide.querySelector('#seed-load');
    if (load) {
      const band = share > .6 ? 'over' : share > .35 ? 'edge' : 'ok';
      const verdict = share > .6
        ? 'Past what the agent can interpret — the context is the constraint now'
        : share > .35
          ? 'More neighborhood than the question needed, and the agent has to rank it'
          : 'Small enough for the agent to read and reason over';
      load.className = `ps-load ${band}`;
      load.innerHTML = '<span class="pl-label">What the agent has to read</span>'
        + `<span class="pl-bar"><i style="width:${Math.round(share * 100)}%"></i></span>`
        + `<span class="pl-count"><b>${returned}</b> of ${view.nodes.length} nodes · <b>${edges}</b> edges returned</span>`
        + `<span class="pl-verdict">${verdict}</span>`;
    }
  }
